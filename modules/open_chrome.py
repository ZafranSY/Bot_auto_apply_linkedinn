...
def createChromeSession(isRetry: bool = False):
    ...
    if stealth_mode:
        ...
        #     print_lg("(Undetected Mode) Got '{}' when using pre-installed ChromeDriver.".format(type(e).__name__)) 
            ...
            # Detect chrome version to prevent version mismatches
            chrome_version = None
            try:
                import subprocess
                import re
                            ...
                            try:
                                ...
                                for item in os.listdir(dir_path):
                                    if re.match(r"^\d+\.\d+\.\d+\.\d+$", item):
                                        chrome_version = int(item.split(".")[0])
                                        break
                                if chrome_version:
                                    break
                            except Exception:
                ...
                print_lg("Failed to automatically detect Chrome version:", e)

            if chrome_version:
                print_lg(f"Forcing ChromeDriver version {chrome_version} to match browser.")
